...
{
    public class StockDetailView : StockListView
    {
        private bool debug = true;
        //private HashSet<Stock> stockObjArray = new HashSet<Stock>();

        public StockDetailView(string p)
        {
            //stockObjArray.Clear();
            ...
            this.Columns.Add(" 涨幅(涨跌价) ", -2, HorizontalAlignment.Right);
        ...
        public void UpdateListData(List<StockType> array)
        {
            ...
            foreach (StockType stock in array)
            {
                ...
                else
                {
                    ...
                    item.SubItems.Add(String.Format("{0:F2}%({1:F2})",
                        (sm.NowPrice - sm.ClosePrice) * 100 / sm.ClosePrice,
                        sm.NowPrice - sm.ClosePrice));
